    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            const Text('Doctor Info', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ...
            _buildInfoRow('Specialty:', doctorInfo?.speciality ?? 'Not available'),
            Align(
              ...
              child: IconButton(
                ...
                onPressed: () async {
                  final updatedDoctor = await Navigator.push<DoctorModel>(
                    context,
                    MaterialPageRoute(
                      builder: (context) => EditDoctorProfile(doctor: doctorInfo!),
                    ),
                  );
  ...
  return Card(
    child: Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          const Text('Clinic Info', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 6),
            ...
            child: IconButton(
              ...
              onPressed: () async {
                final updatedClinic = await Navigator.push<ClinicModel>(
                  context,
                  MaterialPageRoute(
                    builder: (context) => EditClinicInfoProfile(clinic: clinic),
                  ),
                );
                if (updatedClinic != null) {
                  setState(() {
                    int index = clinics.indexWhere((c) => c.clinicId == updatedClinic.clinicId);
                    if (index != -1) {
